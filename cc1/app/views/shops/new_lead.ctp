...
<div>
    <?php echo $this->element('shop_upper_tabs',array('tob_tab' => 'lead_report'));?>
</div>
...
<ul>
   ...
   <?php if(isset($rmlist) && !empty($rmlist)){ ?>
   <li>
       <label for="interest">RM : </label>
       <select id="lead_source" name="rm_list" required="">
            <option>Select RM</option>
            <?php foreach($rmlist as $rm){ ?>
            <option  value="<?php echo $rm['name']; ?>"><?php echo $rm['name']; ?></option>
            <?php } ?>
       </select>
   </li>
   <?php } ?>
	...
	<option selected="selected" value="rmpanel"><?php if($_SESSION['Auth']['show_sd'] == 1){ echo 'Master Distributor'; }else { echo 'RM'; } ?></option>
	<option  value="channeloperations">Channel Ops</option>
        </select> 
   </li>
