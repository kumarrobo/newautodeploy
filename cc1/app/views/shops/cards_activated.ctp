<div>
    ...
    <div id="pageContent" style="min-height:500px;position:relative;">
    	<div class="loginCont">
    		<?php echo $this->element('shop_side_reports',array('side_tab' => 'activate'));?>
    		<div id="innerDiv">
    			<div>
    				...
    				<span id="submit" style="margin-left:30px;"><input type="button" value="Search" style="padding: 0 5px 3px" class="retailBut enabledBut" id="sub" onclick="findActivated();"></span>
    				...
    				<select id="shop">
						...
						<?php foreach($retailers as $retailer) {?>
							<option value="<?php echo $retailer['Retailer']['id'];?>" <?php if(isset($retailer_id) && $retailer_id == $retailer['Retailer']['id']) echo "selected";?>><?php echo $retailer['Retailer']['shopname'] . " - " . $retailer['Retailer']['id'] ; ?></option>
						<?php } ?>
					</select>
					</div>
    				<?php } ?>
				...
				<div class="appTitle">Physical Cards</div>
                    ...
                    <tbody>
                    <?php if(!isset($empty) && empty($activated)) { ?>
                    <tr>
                    	<td colspan="4"><span class="success">No Results Found !!</span></td>
                    </tr>
                    
                    <?php } ?>
                    <?php $i=0; foreach($activated as $active){ 
                    	if($i%2 == 0)$class = '';
                    	else $class = 'altRow';
                    ?>
                      <tr class="<?php echo $class; ?>"> 
			            <td><?php echo date('d-m-Y', strtotime($active['0']['date'])); ?></td>
			             <?php if($_SESSION['Auth']['User']['group_id'] == DISTRIBUTOR) { ?>
			            <td><?php echo $active['Retailer']['shopname']; ?></td>
			            <?php } ?>
			            <td><?php echo $active['Product']['name'] . ' (' . $objShop->shortSerials($active['0']['serials']) . ')'; ?></td>
			            <td class="number"><?php echo count(explode(",",$active['0']['serials'])); ?></td>
    			      </tr>
    			    <?php $i++; } ?> 					    			      
			         </tbody>	         
			   	</table>
	...
	if($('shop')){
		...
		if(!prob){
			if(retailer == 0 && date == -1){
				...
				$('date_err').hide();
				window.location = "http://" + siteName + "/shops/cardsActivated/"+date;
			}
			else{
				$('date_err').hide();
				window.location = "http://" + siteName + "/shops/cardsActivated/"+date+"/"+retailer;
			}
		}
	}
	else {
		if(date_from == '' || date_to == ''){
			...
			window.location = "http://" + siteName + "/shops/cardsActivated/"+date_from+"-"+date_to;
		}
	}
